...
namespace RoomByRoom
{
	public class SelectBonusSystem : IEcsRunSystem
	{
		...
		public void Run(IEcsSystems systems)
		{
			...
			UnitView playerView = _world.GetComponent<UnitViewRef>(player).Value;
			var minDist = float.MaxValue;
			int newBonus = -1;

			foreach (int index in _bonus.Value)
			{
				BonusView bonusView = _world.GetComponent<BonusViewRef>(index).Value;
		...
		{
			_world.AddComponent<SelectCommand>(entity);
			_world.AddComponent<Selected>(entity);
		}

		private void Deselect(int entity)
		{
			_world.DelComponent<Selected>(entity);
			_world.AddComponent<DeselectCommand>(entity);
		}
	}
}